      ...
      <v-col>
        <v-sheet :elevation="24" :width="700" :height="600" :rounded="'xl'" color="green-lighten-3">
          <v-form @submit.prevent="submit">
            ...
            <!-- price -->
            <v-text-field v-model="price" label="price" prefix="$" :rules="priceRules">
            </v-text-field>
            <!-- tags -->
            <v-select label="Listing Tag"
              :items="['Furniture', 'Technology', 'Storage', 'Diningware', 'Textbooks', 'Clothes', 'Accessories', 'Miscellaneous']"
              required v-model="tag">
            </v-select>
            <!-- description -->
            <v-textarea v-model="description" label="Listing Description" :counter="512" :rules="descriptionRules"
              required clearable no-resize>
            </v-textarea>
            <!-- delivery -->
